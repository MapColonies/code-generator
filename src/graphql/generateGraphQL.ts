...
const generateGraphQL = async (targetFilePath: string): Promise<void> => {
  ...
  const addFieldDecorator = (field: IPropGraphQLMapping, classDeclaration: ClassDeclaration): void => {
...
const getType = (mappingType: IDescribeTsType, isParseOptions = false): string => {
...
const parseOptions = (field: IPropGraphQLMapping, targetFile: SourceFile): string[] => {
  ...
  switch (field.mappingType.type) {
    ...
    case PropertiesTypes.CLASS:
      returnType = `(type) => ${parseValue(getType(field.mappingType, true))}`;
      break;
